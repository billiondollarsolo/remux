//! TLS material for the gateway (AW4): load operator-provided PEM cert/key, or
//! generate a self-signed cert for `127.0.0.1`/`localhost` so the gateway works
...
/// The PEM material backing the TLS listener, plus a human-facing note about its
/// provenance (for the startup log).
#[derive(Debug)]
pub struct TlsMaterial {
    ...
    pub key_pem: Vec<u8>,
    /// A short SHA-free fingerprint of the cert PEM, for the startup log so an
    /// operator can pin/verify a self-signed cert.
    pub fingerprint: String,
    /// Whether this cert was generated (self-signed) vs operator-provided.
    pub self_signed: bool,
}

impl TlsMaterial {
    /// Resolve TLS material: if both `cert` and `key` paths are provided, load
    /// them; otherwise generate a self-signed cert for loopback.
...
/// A short, non-cryptographic fingerprint of the cert PEM bytes for logging.
/// (FNV-1a; this is an operator-convenience identifier, not a security control.)
...
mod tests {
    ...
    #[tokio::test]
    async fn self_signed_builds_rustls_config() {
        // A process-level rustls crypto provider must be installed before
        // building a config; the binary does this in `main`.
        let _ = rustls::crypto::aws_lc_rs::default_provider().install_default();
        let m = TlsMaterial::generate_self_signed().expect("generate");
        let _cfg = m.into_rustls_config().await.expect("rustls config");
    }
}
